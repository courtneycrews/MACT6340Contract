...
async function main() {
    ...
    const CourtneyCrewsNFTContractFactory = await ethers.getContractFactory(
        "CourtneyCrewsNFTContract"
    );

    const CourtneyCrewsNFTContract = await CourtneyCrewsNFTContractFactory.deploy(
        args.mint_price,
        args.max_tokens,
        args.base_uri,
        args.royaltyArtist,
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await CourtneyCrewsNFTContract.mintTo(ipfs[i], {
